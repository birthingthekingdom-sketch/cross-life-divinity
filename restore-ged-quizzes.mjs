...
console.log('Starting GED Quiz Restoration...\n');

// Get all GED lessons with their IDs, ordered by course and lesson order
const [lessons] = await connection.execute(`
  SELECT l.id, l.title, l.lessonOrder, c.code, c.id as courseId
  FROM lessons l 
  JOIN courses c ON l.courseId = c.id 
  WHERE c.code LIKE 'GED%' 
  ORDER BY c.code, l.lessonOrder
`);

console.log(`Found ${lessons.length} GED lessons in database\n`);

// Create a mapping of lesson titles to their IDs
const lessonMap = {};
lessons.forEach(lesson => {
  lessonMap[lesson.title.toLowerCase()] = lesson.id;
});

// GED MATH Quiz Data
const gedMathQuizzes = {
...
// Insert GED Math quizzes
for (const [lessonTitle, questions] of Object.entries(gedMathQuizzes)) {
  const lessonId = lessonMap[lessonTitle.toLowerCase()];
  ...
  for (let i = 0; i < questions.length; i++) {
    ...
    try {
      ...
      console.error(`Error inserting quiz for lesson ${lessonTitle}:`, error.message);
    }
  }
  
  console.log(`✅ Added ${questions.length} quizzes for lesson: ${lessonTitle}`);
}

console.log(`\n✅ Total GED quiz questions inserted: ${totalInserted}`);
