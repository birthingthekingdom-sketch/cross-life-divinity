...
async function seedLessons() {
  try {
    console.log('Starting GED lessons seeding...\n');
    
    // Get all GED courses
    const gedCourses = await db
      .select()
      .from(courses)
      .where(eq(courses.code, 'GED-MATH'))
      .limit(1);
    
    console.log('Fetching GED courses...');
    
    let totalAdded = 0;
    
    for (const [courseCode, lessonsList] of Object.entries(GED_LESSONS)) {
      // Get course by code
      ...
      if (!courseResult || courseResult.length === 0) {
        console.log(`⚠ Course ${courseCode} not found, skipping...`);
        continue;
      }
      
      const courseId = courseResult[0].id;
      console.log(`\nProcessing ${courseCode} (ID: ${courseId}) with ${lessonsList.length} lessons...`);
      ...
      await db
        .update(courses)
        .set({ totalLessons: lessonsList.length })
    ...
    console.log('✓ GED lessons seeding completed successfully!');
