...
import { 
  ...
  pendingWrittenAnswers, InsertPendingWrittenAnswer, PendingWrittenAnswer
} from "../drizzle/schema";
...
// ============================================================================
// PENDING WRITTEN ANSWERS MANAGEMENT
// ============================================================================

/**
 * Create pending written answer record
 */
export async function createPendingWrittenAnswer(data: InsertPendingWrittenAnswer) {
  const db = await getDb();
  if (!db) throw new Error("Database not available");

  await db.insert(pendingWrittenAnswers).values(data);
}

/**
 * Get pending written answers for admin review (not yet graded)
 */
export async function getPendingWrittenAnswers() {
  const db = await getDb();
  if (!db) return [];

  return db.select().from(pendingWrittenAnswers)
    .where(eq(pendingWrittenAnswers.status, "pending"))
    .orderBy(desc(pendingWrittenAnswers.submittedAt));
}

/**
 * Get pending written answers count for admin
 */
export async function getPendingWrittenAnswersCount() {
  const db = await getDb();
  if (!db) return 0;

  const result = await db.select({ count: sql<number>`COUNT(*)` })
    .from(pendingWrittenAnswers)
    .where(eq(pendingWrittenAnswers.status, "pending"));
...
/**
 * Get pending written answers by course
 */
export async function getPendingWrittenAnswersByCourse(courseId: number) {
  const db = await getDb();
  if (!db) return [];

  return db.select().from(pendingWrittenAnswers)
    .where(and(
      eq(pendingWrittenAnswers.courseId, courseId),
      eq(pendingWrittenAnswers.status, "pending")
    ))
    .orderBy(desc(pendingWrittenAnswers.submittedAt));
}

/**
 * Get pending written answers by student
 */
export async function getPendingWrittenAnswersByStudent(userId: number) {
  const db = await getDb();
  if (!db) return [];

  return db.select().from(pendingWrittenAnswers)
    .where(and(
      eq(pendingWrittenAnswers.userId, userId),
      eq(pendingWrittenAnswers.status, "pending")
    ))
    .orderBy(desc(pendingWrittenAnswers.submittedAt));
}
...
/**
 * Get written answer by ID
 */
export async function getPendingWrittenAnswerById(id: number) {
  const db = await getDb();
  if (!db) return null;

  const result = await db.select().from(pendingWrittenAnswers)
    .where(eq(pendingWrittenAnswers.id, id))
    .limit(1);
...
export async function gradeWrittenAnswer(
  ...
  await db.update(pendingWrittenAnswers)
    .set({
      status: "graded",
      adminScore: score,
...
/**
 * Get graded written answers for a student
 */
export async function getGradedWrittenAnswersByStudent(userId: number) {
  const db = await getDb();
  if (!db) return [];
  ...
  return db.select().from(pendingWrittenAnswers)
    .where(and(
      eq(pendingWrittenAnswers.userId, userId),
      eq(pendingWrittenAnswers.status, "graded")
    ))
    .orderBy(desc(pendingWrittenAnswers.gradedAt));
}

/**
 * Get written answers by quiz submission
 */
export async function getWrittenAnswersBySubmission(quizSubmissionId: number) {
  const db = await getDb();
  if (!db) return [];

  return db.select().from(pendingWrittenAnswers)
    .where(eq(pendingWrittenAnswers.quizSubmissionId, quizSubmissionId))
    .orderBy(desc(pendingWrittenAnswers.submittedAt));
}
