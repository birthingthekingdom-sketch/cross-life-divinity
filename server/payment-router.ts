...
export const paymentRouter = router({
  ...
  /**
   * Create checkout session for 3-course bundle
   */
  createBundleCheckout: protectedProcedure
    .input(
      z.object({
        courseIds: z.array(z.number()).length(3),
      })
    )
    .mutation(async ({ ctx, input }) => {
      const user = ctx.user;
      ...
      const courses = await Promise.all(
        input.courseIds.map(id => db.getCourseById(id))
      );

      if (courses.some(c => !c)) {
        throw new TRPCError({ code: "NOT_FOUND", message: "One or more courses not found" });
      }

      // Check if already enrolled in any of these courses
      const enrollments = await Promise.all(
        input.courseIds.map(id => db.isUserEnrolledInCourse(user.id, id))
      );

      if (enrollments.some(e => e)) {
        throw new TRPCError({ code: "BAD_REQUEST", message: "Already enrolled in one or more of these courses" });
      }
      ...
      const session = await stripe.checkout.sessions.create({
        ...
        line_items: [
          {
            price_data: {
              ...
              product_data: {
                ...
                description: `Choose any 3 courses: ${courses.map(c => c!.title).join(", ")}`,
              },
        ...
        success_url: `${ctx.req.headers.origin}/dashboard?bundle=success`,
        cancel_url: `${ctx.req.headers.origin}/pricing?bundle=canceled`,
        client_reference_id: user.id.toString(),
        metadata: {
          userId: user.id.toString(),
