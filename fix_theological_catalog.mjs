...
// Correct theological course list
...
async function fixCatalog() {
  ...
  try {
    ...
    console.log('Fixing theological course catalog...\n');
    
    // Get all current theological courses
    const [currentCourses] = await conn.query(
      "SELECT id, code, title FROM courses WHERE courseType = 'theological' ORDER BY code"
    );
    ...
    console.log('Updating/Creating theological courses:');
