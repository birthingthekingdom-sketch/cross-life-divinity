...
async function populateLessons() {
  ...
  try {
    console.log('Starting GED lessons population...');
    
    // Get GED course IDs
    const [courses] = await connection.query(
      'SELECT id, code FROM courses WHERE code IN (?, ?, ?, ?)',
      ['GED-MATH', 'GED-RLA', 'GED-SCI', 'GED-SS']
    );
    
    console.log(`Found ${courses.length} GED courses`);
    
    let totalLessonsAdded = 0;
    
    for (const course of courses) {
      const courseCode = course.code;
      const lessonsList = GED_LESSONS[courseCode] || [];
      
      console.log(`\nProcessing ${courseCode} with ${lessonsList.length} lessons...`);
      ...
      for (let i = 0; i < lessonsList.length; i++) {
        ...
        await connection.query(
          ...
          [course.id, lesson.title, lesson.content, i + 1]
        );
        totalLessonsAdded++;
      }
      ...
      await connection.query(
        ...
        [lessonsList.length, course.id]
      );
      
      console.log(`✓ Added ${lessonsList.length} lessons to ${courseCode}`);
    }
    ...
    const [testPayments] = await connection.query(
      'SELECT COUNT(*) as count FROM stripe_customers WHERE customerId LIKE ?',
      ['test_%']
    );
    
    if (testPayments[0].count > 0) {
      await connection.query('DELETE FROM stripe_customers WHERE customerId LIKE ?', ['test_%']);
      console.log(`✓ Deleted ${testPayments[0].count} test Stripe customers`);
    }
